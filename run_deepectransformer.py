...
if __name__ == "__main__":
    ...
    failed_cases = save_dl_result(
        y_pred, y_score, input_ids, explainECs, output_dir + "/tmp"
    )
    ...
    if len(failed_cases) > 0:
        ...
        run_blastp(
            blastp_input,
            blastp_tmp_output,
            "./model/swissprot_enzyme_diamond",
            threads=num_cpu,
        )
        ...
        with open(blastp_output, "w") as fp:
            ...
            for seq_id in blastp_pred:
                ec = blastp_pred[seq_id]
                fp.write(f"{seq_id}\t{ec}\n")

        merge_predictions(
            f"{output_dir}/tmp/DL_prediction_result.txt", blastp_output, output_dir
        )

    else:
        shutil.copy(output_dir + "/tmp/DL_prediction_result.txt", output_dir)
        os.rename(
            output_dir + "/DL_prediction_result.txt",
            output_dir + "/DeepECv2_result.txt",
        )
