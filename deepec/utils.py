...
def argument_parser(version=None):
    ...
    parser.add_argument(
        ...
        "--seq_file",
        required=False,
        default="./Dataset/uniprot_dataset.fa",
        help="Sequence data",
    )
    parser.add_argument("-o", "--output_dir", required=True, help="Output directory")
    parser.add_argument(
        "-e", "--epoch", required=False, type=int, default=30, help="Total epoch number"
    )
    parser.add_argument(
        "-b", "--batch_size", required=False, type=int, default=32, help="Batch size"
    )
    parser.add_argument(
        "-r",
        "--learning_rate",
    ...
    parser.add_argument(
        "-g", "--gpu", required=False, default="cuda:0", help="Specify gpu"
    )
    parser.add_argument(
        "-cpu",
        "--cpu_num",
...
def draw(avg_train_losses, avg_valid_losses, output_dir, file_name="CNN_loss_fig.png"):
    ...
    plt.axvline(
        min_position, linestyle="--", color="r", label="Early stopping checkpoint"
    )
